    ...
    function loadLogs() {
        fetch('/api/logs?limit=100')
            ...
            .then(function (logs) {
                ...
                if (logs.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:var(--text-muted);padding:2rem;">No logs recorded yet.</td></tr>';
                    return;
                }
                ...
                logs.forEach(function (log) {
                    ...
                    var site = (log.detected_site || '').trim() || '—';
                    var dest = (log.redirect_destination || '').trim();
                    var destPreview = dest ? (dest.length > 40 ? dest.substring(0, 40) + '...' : dest) : '—';
                    ...
                    html += '<tr>' +
                        ...
                        '<td style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' + escapeHtml(dest) + '">' + escapeHtml(destPreview) + '</td>' +
